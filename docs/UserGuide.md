...
* **EMAIL** (`e/`, required): `local@domain` format. Max 254 characters. Automatically lowercased. The local part may contain letters, digits, and `+ _ . -`; must start and end with a letter or digit; no consecutive special characters (e.g. `a..b@x.com` is invalid). The domain must have at least one `.` and a TLD of at least two letters (e.g. `.com`, `.in`). Strips leading/trailing whitespace.
